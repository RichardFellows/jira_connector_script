...
def __():
    ...
    return datetime, duckdb, go, json, make_subplots, mo, os, pd, px, subprocess, sys, timedelta
...
def __():
    ...
    incremental_update = mo.ui.checkbox(
        label="Incremental update (only fetch changed issues)",
        value=True
    )
...
@app.cell
def __(auth_method, datetime, incremental_update, jira_url, mo, password_input, refresh_date_range, refresh_project, subprocess, sys, token_input, username_input):
    # Data refresh trigger
    refresh_button = mo.ui.button(
        label="🔄 Refresh JIRA Data",
        kind="success",
        disabled=not (jira_url.value and refresh_project.value and 
                     (token_input.value if auth_method.value == "token" else 
                      (username_input and password_input and username_input.value and password_input.value)))
    )
    ...
    if refresh_button.value:
        try:
            ...
            if auth_method.value == "token":
                ...
                cmd.extend(["--username", username_input.value, "--password", password_input.value])
            ...
            if result.returncode == 0:
                refresh_status._update(f"✅ Data refresh completed successfully! {datetime.now().strftime('%H:%M:%S')}")
            else:
    ...
    mo.vstack([
        refresh_button,
        refresh_status if refresh_status.value else mo.md("")
    ])
...
def __(db_path, duckdb, json, pd, refresh_status):
    ...
    try:
        ...
        # Load extraction log
        log_df = conn.execute("SELECT * FROM extraction_log ORDER BY extraction_time DESC").df()
        ...
        if not issues_df.empty and 'custom_fields' in issues_df.columns:
            issues_df['story_points'] = issues_df['custom_fields'].apply(
                lambda x: json.loads(x).get('Story Points') if x and x != 'null' else None
            )
        ...
        if not log_df.empty and 'extraction_time' in log_df.columns:
            ...
            last_refresh = f" (Last updated: {last_extraction.strftime('%Y-%m-%d %H:%M:%S')})"
...
def __(filtered_df, mo):
    ...
    issue_types = mo.ui.multiselect(
        options=list(filtered_df['issue_type'].unique()) if not filtered_df.empty else [],
...
def __(analysis_df, mo, px):
    ...
    if analysis_df.empty:
        ...
        if not velocity_df.empty:
            ...
            monthly_velocity = velocity_df.groupby(['project_key', 'month'])['story_points'].sum().reset_index()
...
def __(analysis_df, mo, px):
    ...
    if analysis_df.empty:
        ...
        estimation_df = analysis_df[
            (analysis_df['story_points'].notna()) & 
            (analysis_df['resolved'].notna())
        ].copy()
        
        if not estimation_df.empty:
            estimation_df['cycle_time_days'] = (estimation_df['resolved'] - estimation_df['created']).dt.days
            ...
            fig_estimation = px.scatter(
                ...
                labels={'story_points': 'Story Points (Estimate)', 'cycle_time_days': 'Cycle Time (Days)'}
            )
            fig_estimation.update_layout(height=500)
            
            mo.ui.plotly(fig_estimation)
        else:
            mo.md("No data available with both story points and resolution dates for estimation accuracy analysis.")
...
def __(analysis_df, go, make_subplots, mo):
    ...
    if not analysis_df.empty:
        ...
        fig_dist = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Issues by Status', 'Issues by Type', 'Issues by Priority', 'Issues by Project'),
        ...
        fig_dist.add_trace(
            go.Pie(labels=project_counts.index, values=project_counts.values, name="Project"),
            row=2, col=2
        )
...
def __(analysis_df, mo, px, resolved_df):
    ...
    if not resolved_df.empty:
        ...
        cycle_trend_df['resolved_month'] = cycle_trend_df['resolved'].dt.to_period('M').astype(str)
        avg_cycle_time = cycle_trend_df.groupby('resolved_month')['cycle_time_days'].mean().reset_index()
        ...
        fig_cycle_trend = px.line(
            ...
            labels={'cycle_time_days': 'Average Cycle Time (Days)', 'resolved_month': 'Month'}
        )
        fig_cycle_trend.update_layout(height=400)
        
        mo.vstack([
            mo.ui.plotly(fig_cycle),
            mo.ui.plotly(fig_cycle_trend)
        ])
    else:
...
def __(analysis_df, mo):
    ...
    if not analysis_df.empty:
        ...
        avg_story_points = analysis_df['story_points'].mean() if 'story_points' in analysis_df.columns else 0
        total_story_points = analysis_df['story_points'].sum() if 'story_points' in analysis_df.columns else 0
        
        avg_cycle_time = analysis_df[analysis_df['resolved'].notna()]['cycle_time_days'].mean() if 'cycle_time_days' in analysis_df.columns else 0
...
def __(log_df, mo, pd):
    ...
    if not log_df.empty:
        ...
        if 'extraction_time' in display_log.columns:
            display_log['extraction_time'] = pd.to_datetime(display_log['extraction_time']).dt.strftime('%Y-%m-%d %H:%M:%S')
        
        # Show recent extractions
        recent_extractions = display_log.head(10)[['project_key', 'start_date', 'end_date', 'issues_extracted', 'extraction_time']]
        
        mo.ui.table(
            recent_extractions,
            label="Recent Data Extractions (Last 10)"
        )
    else:
